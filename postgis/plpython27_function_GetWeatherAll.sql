...
## IMPORTANT to make this work replace the string xyzapikey (on line 9 in the api call to openweathermap) with your own key string
...
CREATE OR REPLACE FUNCTION GetWeatherAll(lon float, lat float) RETURNS weather_values AS $$
  ...
  if int(js_data['id'])!=0: # check if we have at least a weather station
    ...
    if 'main' in station:
     ...
     temperature = -9999.6
  data_city = urllib2.urlopen('http://api.openweathermap.org/data/2.5/weather?id=%s&appid=de429a99346fd7fda262c49492046487' % (cityid))
